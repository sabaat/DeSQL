...
class SubQueryGeneratorVisitor(var enabled: Boolean = false) extends SubQueryVisitor {
  ...
  def visit(aggregate: Aggregate, subQueryList: ArrayBuffer[SubQueryStorage]): ArrayBuffer[SubQueryStorage] = {
    ...
    newSubQueryList.append(new SubQueryStorage("Select * from " + aggregate.child.asInstanceOf[SubqueryAlias].identifier.toString(), aggregate.child.asInstanceOf[SubqueryAlias].child.getMappingIndex))
